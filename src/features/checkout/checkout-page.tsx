...
export function CheckoutPage({ branches }: { branches: Branch[] }) {
  ...
  // Prefill contact from the (mock) customer profile once, if blank.
  useEffect(() => {
    const patch: Partial<typeof checkout> = {};
    if (!checkout.phone && customerPhone) patch.phone = customerPhone;
    if (!checkout.email && customerEmail) patch.email = customerEmail;
  ...
  const sameProvince = !!checkout.address.province && branch?.city === checkout.address.province;
  ...
  const canPlace = okLines.length > 0 && oosCount === 0 && contactOk;
  const invalidHint =
    oosCount > 0
  ...
  if (placed) {
    const paymentLabel =
      PAYMENT_METHODS.find((m) => m.id === checkout.paymentMethodId)?.label ?? "—";
    return (
      <OrderSuccess
        orderId={placed.id}
        total={placed.total}
